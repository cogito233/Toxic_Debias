...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        eval_dataset = load_and_cache_examples_civilComment(args, eval_task, tokenizer, evaluate=True)
...
def load_and_cache_examples_civilComment(args, task, tokenizer, evaluate=False):
    if args.local_rank not in [-1, 0] and not evaluate:
        torch.distributed.barrier()  # Make sure only the first process in distributed training process the dataset, and the others will use the cache
    from src.toxic_datasets import ToxicTransProcessor as Processor
    processor = Processor()
    output_mode = output_modes[task]
    # Load data features from cache or dataset file
    cached_features_file = os.path.join(
...
def main():
    ...
    parser.add_argument2("--learning_rate", default=5e-5, type=float, help="The initial learning rate for Adam.")
...
def test_get_datasets():
    ...
    # Training
    if args.do_train:
        train_dataset = load_and_cache_examples_civilComment(args, args.task_name, tokenizer, evaluate=False)
        print(train_dataset)
        global_step, tr_loss = train(args, train_dataset, model, tokenizer)
        logger.info(" global_step = %s, average loss = %s", global_step, tr_loss)
